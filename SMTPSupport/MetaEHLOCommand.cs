...
namespace SMTPSupport
{
    internal class MetaEHLOCommand : SMTPCommand
    {
        ...
        {
            if( !firstLine.StartsWith( "!EHLO" ) && !firstLine.StartsWith( "!HELO" ) ) throw new ArgumentException( "Must start with !EHLO or !HELO." );
            ...
            if (firstLine.Substring(5).Trim() != null && firstLine.Substring(5).Trim() != "")
            {
                if (firstLine.Substring(5).Trim().Contains(" "))
                    ...
                    return new SMTPCommandParseResult(new MetaEHLOCommandToExecute( username, password ));
            }
